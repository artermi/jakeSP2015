...
int csiebox_client_run(csiebox_client* client) {
  ...
  if (!prepare_and_sync(client)) {
    fprintf(stderr, "upload sync fail\n");
    return 0;
  }
...
static void sync_file_data(
  ...
  int again = 0;
  do {
    lock.l_type = F_WRLCK; // test wrlck
...
static void monitor_home(csiebox_client* client) {
  ...
  fdmax = 4; // std: 0,1,2 conn:3 ino:4
  fprintf(stderr, "client->conn_fd: %d\n", client->conn_fd);
  fprintf(stderr, "client->inotify_fd: %d\n", client->inotify_fd);
  ...
  fprintf(stderr, "do sub-loop\n");
    for(i = 0; i <= fdmax; i++){
  fprintf(stderr, "\ti in sub loop: %d\n", i);
      if(FD_ISSET(i, &read_fds)){
  fprintf(stderr, "FD_ISSET\n");
        if(i == client->inotify_fd){
  fprintf(stderr, "do handle_inotify\n");
          handle_inotify(client);
